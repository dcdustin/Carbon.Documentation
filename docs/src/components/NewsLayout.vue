...
<template>
  <VPHomeContent>
    <div v-if="frontmatter" class="fixed inset-0 overflow-hidden bg-neutral-950/100">
    ...
    <div class="relative">
      <div class="mx-auto my-40 max-w-screen-lg space-y-6">
        <div class="text-center text-5xl font-black uppercase" @click.stop>
          ...
          <div class="my-3 block">
            <VPBadge type="info">{{ formatDate(frontmatter.date).string }}</VPBadge>
            <VPBadge v-for="tag in frontmatter.tags" :key="tag" type="tip">{{ tag }}</VPBadge>
          </div>
          {{ frontmatter.title }}
        </div>
