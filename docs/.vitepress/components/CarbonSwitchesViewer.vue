...
<template>
  <AsyncState :isLoading="isLoading" :error="error" loadingText="Loading switches...">
      ...
      <div class="flex flex-col gap-5 mt-4">
        <InfinitePageScroll :list="filteredSwitches" :pageSize="pageSize" v-slot="{ renderedList }">
          <div class="fixed bottom-4 sm:right-4 sm:left-auto left-1/2 z-10">
            ...
            >
              Rendering {{ renderedList.length }} of {{ filteredSwitches.length }} filtered switches,
              {{ switches.length }} total switches.
            </div>
          </div>
    ...
    <div v-else class="py-8 flex flex-col items-center justify-center gap-2">
      ...
      <p v-if="switches && switches.length == 0" class="text-sm">
